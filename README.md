...
**工具结果的截断规则**：`max_tool_result_chars`（默认 8000）是**兜底**，管的是那些顺带
返回点内容的工具。`http_request` 和 `read_file` 不走它——这两个工具的正文就是答案本身，
砍掉一半比没有更糟：模型会拿半截内容当完整的用，还照样据此下结论。它们的长度各自有交代：

- `http_request`：响应体整份返回，头部报出 `正文长度: N 字符`；嫌大就换个更精确的 URL。
- `read_file`：由显式的 `max_bytes` 参数控制（默认 100000），模型看得见也调得大。

> 代价要说清楚：正文不再截断，一个大页面就是一整份进上下文。`max_context_chars`
> 的裁剪以**整轮**为单位，所以超大结果不会把上下文切成碎片，但会挤掉更早的对话轮次。
